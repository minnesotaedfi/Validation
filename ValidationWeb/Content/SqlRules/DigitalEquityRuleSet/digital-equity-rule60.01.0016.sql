...
error when no record for same student where SEOA.StudentIndicator.IndicatorName = PrimaryLearningDeviceProvider

*/

DECLARE @RuleId VARCHAR(32) = '60.01.0016';
DECLARE @Message NVARCHAR(MAX) = 'When Digital Device is not reported as None, no PrimaryLearningDeviceProvider should not be reported for this student.

If SEOA.StudentIndicator.IndicatorName = DigitalDevice and SEOA.StudentIndicator.Indicator <> None, 
error on no additional record for same student where SEOA.StudentIndicator.IndicatorName = PrimaryLearningDeviceProvider';
