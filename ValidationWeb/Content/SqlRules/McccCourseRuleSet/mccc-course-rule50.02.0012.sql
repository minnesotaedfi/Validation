...
When Course grade level offered are within grades K-8, If G is selected, then NOT B, E, X, A, C, D

*/

DECLARE @RuleId VARCHAR(32) = '50.02.0012';
DECLARE @Message NVARCHAR(MAX) = 'Course level type combination error detected for course offered for grades K-8. if course level type G is selected for the course, B, E, X, A, C, or D cannot also be selected on the same course.';
